$(function() {
	...
	// Add links to Kenginefile directive tokens in code blocks.
	// See include/docs-head.html for the whitelist bootstrapping logic
	$('pre.chroma .k')
		.filter((k, item) =>
			window.KenginefileDirectives.includes(item.innerText)
				|| item.innerText === '<directives...>'
		)
		.map(function(k, item) {
			let text = item.innerText;
			let url = text === '<directives...>'
				? '/docs/kenginefile/directives'
				: '/docs/kenginefile/directives/' + text;
	...
	$('pre.chroma .nd')
		.map(function(k, item) {
			...
			$(item).html('<a href="/docs/kenginefile/matchers#syntax" style="color: inherit;" title="Matcher token">' + text + '</a>');
		});
});
