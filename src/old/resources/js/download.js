...
$.get("/api/packages")
	.done(function (json) {
		...
		const preselectedPackage = new URL(
		...
		$(function () {
			...
			for (var i = 0; i < packageList.length; i++) {
				...
				if (provider) {
					var $pkgHost = $('<span class="package-host"/>').text(
						provider
					);
				...
				if (pkg.modules && pkg.modules.length > 0) {
					for (var j = 0; j < pkg.modules.length; j++) {
						...
						$(".module-link", $mod)
							.attr("href", "/docs/modules/" + mod.name)
							.text(mod.name)
							.attr("title", "View module details");
						$(".module-desc", $mod).text(
							moduleDocsPreview(mod, 120)
						);
	...
	.fail(function (jqxhr, status, error) {
		swal({
			...
			title: "Unavailable",
			content: $(
				'<div>Sorry, the build server is down for maintenance right now. You can try again later or <a href="https://github.com/khulnasoft/kengine/releases/latest">download pre-built Kengine binaries from GitHub</a> any time.</div>'
			)[0],
		});
		$(function () {
			disableFields(false);
...
$(function () {
	...
	$("#filter").on("search keyup", function (event) {
		...
		$(".package").each(function () {
			...
			var corpus = $(this)
				.find(".package-link, .module-link, .module-desc")
				.text()
				.trim()
				.toLowerCase();
	...
	$("#optional-packages").on("click", ".package", function () {
		...
		currentSelected.forEach((selected) =>
			newUrl.searchParams.append("package", selected)
		);
	...
	$("#optional-packages").on(
		"click",
		".package-link, .module-link, .package-version-input",
		function (event) {
	...
	$("#download").click(function () {
		...
		$.ajax($(this).attr("href"), { method: "HEAD" })
	...
	if (jqxhr.status == 502) {
		swal({
			...
			content: $content.html(
				'Sorry, the build server is down for maintenance right now. You can try again later or <a href="https://github.com/khulnasoft/kengine/releases/latest">download pre-built Kengine binaries from GitHub</a>.'
			)[0],
		});
	} else {
		swal({
			...
			title: "Build failed",
			content: $content.html(
				'The two most common reasons are:<ol><li><b>A plugin is not compiling.</b> The developer must release a new version that compiles.</li><li><b>The build configuration is invalid.</b> If you specified any versions, make sure they are correct and <a href="https://golang.org/cmd/go/#hdr-Module_compatibility_and_semantic_versioning" target="_blank">within the same major version</a> as the path of the associated package.</li></ol>In the meantime, you can <a href="https://github.com/khulnasoft/kengine/releases/latest">download Kengine from GitHub</a> without any plugins.'
			)[0],
		});
	}
}
